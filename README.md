...
- **Today** — In-Patient toggle banner at the top, missed-dose banner, chemo banner (when applicable), dose counters, vitals inputs, quick-log cards, and a grouped "Evening meds" card with a one-tap "Take all"
- **History** — grouped per day into Overnight/Morning/Afternoon/Evening; per-day summaries and rows exclude Weight, Temperature, and In-Patient entries (which have their own views); Period Start/End rows appear but aren't counted as doses
- **Weight** — weight trend chart, plus the Menstrual Cycle card
- **In-Patient** — grouped date ranges of hospital stays
