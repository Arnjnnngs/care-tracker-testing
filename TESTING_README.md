...
| v67 | Jul 23, 2026 | v66 | **Correction: reverted the v61/v62 Zofran "fix" — the 3-day chemo block was correct all along.** Aaron confirmed the original, correct spec directly: Zofran is blocked for chemo day plus the 2 days after (3 days total — e.g. chemo Thursday blocks Thu/Fri/Sat, opens Sunday 8 AM), exactly matching `o >= 0 && o <= 2` — the logic this repo shipped with before v61. During the 30-use-case QA pass, this was misdiagnosed as a bug because this file's own "Chemo Cycle" section and `TESTING_HANDOFF.md` both incorrectly documented a 2-day block — the *documentation* was wrong, not the code. v61 changed `zofranBlockedOn()` to `o >= 0 && o <= 1` and v62 changed the displayed unlock time to match; both reverted here back to the original 3-day logic. **Production `care-tracker` was never touched and was correct the entire time** — the earlier flag in this file and in `TESTING_HANDOFF.md` claiming production has "the same bug" was itself incorrect and has been removed. `test_zofran_chemo_window.js` rewritten to lock in the 3-day spec (8/8) plus a full regression pass (223/223 across all suites). | Testing |
